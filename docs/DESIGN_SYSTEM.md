...
- **Todo el display de fases estaba muerto** desde que A6 borró el modelo `Phase` legacy (2026-07-14, encontrado al hacer S1): `KnockoutBracket`, `PublicStandingsSection`, `AdminStandingsSection` y el badge de `TournamentDetailView` filtraban por `match.phase?.name` contra nombres del enum viejo (`OCTAVOS_DE_FINAL`, `CRUCES`…), pero la query trae `tournamentPhase` y nunca `phase` → el campo era siempre `undefined`, el bracket renderizaba **cero** partidos y el badge de fase no aparecía nunca. El tipo `IMatch` mentía (declaraba `phase`, que la API no devuelve) y por eso TypeScript no lo agarró. Corregido: se filtra por `tournamentPhase.type === "KNOCKOUT"` y los helpers legacy se borraron.
