...
| `FilterSelect` / `FilterGrid` | `FilterSelect.tsx` | Filtro desplegable (patrón §6 de UI_PATTERNS): disparador de 44px que **muestra el valor activo sin abrirlo**, y `Popover` + `Command` con la lista; buscador automático a partir de 8 opciones. `FilterGrid` los apila en mobile y los acomoda en columnas cuando hay lugar. **Reemplazó a los chips de F2 (2026-07-22)**: con 14 categorías + 9 estados, la fila de píldoras tapaba los resultados en el celular y el scroll horizontal escondía opciones sin avisar que había más. Va de la mano con `useUrlFilters` (`hooks/use-url-filters.ts`), que mantiene el estado en la query de la URL. |
...
| `NumberTicker` | `NumberTicker.tsx` | **Contador animado de KPI** (F4, 2026-07-14): anima de 0 al valor en 700ms. Ya viene dentro de `StatCard` para valores numéricos — no lo agregues a mano ahí. **Solo para KPIs:** en una tabla o un marcador el número tiene que leerse al instante. Respeta `prefers-reduced-motion` vía `usePrefersReducedMotion()` (el CSS global no alcanza: esto es `requestAnimationFrame`). |
| `Breadcrumbs` | `Breadcrumbs.tsx` | **Ruta de navegación de las subpáginas admin** (F3, 2026-07-14). Agrega el nivel "Panel" solo — no lo pases en `items`; el último ítem es el actual y no lleva link. `PageHeader` lo renderiza por dentro con su prop `breadcrumbs`; las pantallas de detalle, que tienen header propio, lo usan directo. Reemplazó a los botones "Volver a X" de las 6 subpáginas. |
| `DataTable` | `DataTable.tsx` | **Tabla del panel** (F3): orden por columna, búsqueda, filtros desplegables, paginación, colapso a cards en mobile y estado vacío. Ver §4 — no armes una `<Table>` a mano. |
| `FormSheet` + campos | `form/FormSheet.tsx`, `form/fields.tsx` | **Formulario de alta/edición del panel** (F3, 2026-07-14): panel lateral en desktop / pantalla completa en mobile, header y acciones sticky, loading de submit, resumen de errores, guarda de cambios sin guardar y borradores. Los campos (`TextField`, `NumberField`, `DateField`, `TextareaField`, `SelectField`, `SwitchField`, `ImageField`, `ColorField`) + `FormSection`/`FieldRow` viven en `form/fields.tsx`. Ver UI_PATTERNS §11 — no armes un `Dialog` con inputs a mano. |
...
**Hooks compartidos** (`hooks/`): `useUrlFilters` (filtros en la query de la URL, ver UI_PATTERNS §6), **`usePrefersReducedMotion`** (`use-reduced-motion.ts`, F4 — para animaciones hechas en JS, que la media query global no puede frenar), **`useFormDraft`** (`use-form-draft.ts`, F3 — autoguardado de borradores en `localStorage` para formularios de alta; ver UI_PATTERNS §11) y **`useIsMounted`** (`use-mounted.ts`, 2026-07-14) — guard de hidratación con `useSyncExternalStore`: `false` en el server y durante la hidratación, `true` después. Usalo para todo lo que solo puede resolverse en el browser (tema, `localStorage`, `window`); reemplaza al viejo `useState(false)` + `useEffect(() => setMounted(true))`, que dispara un render en cascada.
...
Al agregar una ruta admin nueva: sumá un objeto a **`adminNavItems`** ([lib/constants/admin-nav.ts](../lib/constants/admin-nav.ts) — `title`/`href`/`icon` Lucide/`roles`/`enabled`/`keywords`) y aparece en el sidebar **y** en el command palette. Es la fuente única de la navegación del panel: no crees una navegación paralela ni copies la lista.

**Command palette (F3, 2026-07-14):** `Ctrl/⌘+K` abre `components/admin/CommandPalette.tsx`, montado por `AdminShell` junto a su disparador visible en el header (un atajo que no se anuncia no existe para quien no lo conoce). Hace dos cosas: **busca recursos** (torneos, equipos y jugadores por nombre contra `GET /api/admin/search`, acotado a las organizaciones del usuario vía `getPanelOrgIds` — N3) y **salta a secciones** (de `adminNavItems`, filtradas por rol). El filtrado propio de cmdk se apaga (`shouldFilter={false}`) mientras hay resultados del server: ya vienen filtrados por la query. La primitiva `components/ui/command.tsx` es la de shadcn con una desviación deliberada: `CommandDialog` reenvía sus props extra al `<Command>` (no al `<Dialog>`), que es lo que permite pasar `shouldFilter`.
