...
- **Componentes compartidos primero (F0):** hero público → `<PageHero>`; header admin → `<PageHeader>`; KPI admin → `<StatCard>`; badge de estado → `<StatusBadge entity=... status=...>`; estado vacío → `<EmptyState>`; loading con forma conocida → `<SkeletonTable>`/`<SkeletonCards>`; confirmación destructiva → `<ConfirmDialog>`. Todos en `components/shared/` (ver COMPONENT_LIBRARY.md §2b) — no reescribas ese markup a mano.
