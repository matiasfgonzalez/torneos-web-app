...
import { Award, Users, Activity, UserX, TrendingUp } from "lucide-react";
...
const StatsCards = (props: PropsStatsCards) => {
  ...
  const stats = [
    ...
    {
      title: "Suspendidos",
      value: players.filter((p) => p.status === "SUSPENDED").length,
      icon: UserX,
                ...
                <div
                  ...
                  style={{
                    width:
                      players.length > 0
                        ? `${Math.min(
                            (stat.value / Math.max(players.length, 1)) * 100,
                            100,
                          )}%`
                        : "0%",
                  }}
