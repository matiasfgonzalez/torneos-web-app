        ...
        {teamData.jugadores.length === 0 ? (
          <Card className="relative bg-white dark:bg-gray-900/80 shadow-2xl border border-gray-100 dark:border-gray-800 overflow-hidden rounded-2xl">
            ...
            <CardContent className="text-center py-16">
              <div className="w-20 h-20 mx-auto mb-4 bg-gradient-to-br from-brand/20 to-brand-mid/10 rounded-full flex items-center justify-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            {teamData.jugadores.map((jugador) => (
              <div
                key={jugador.id}
                className="group relative bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 border border-gray-100 dark:border-gray-700 hover:border-brand/50 hover:-translate-y-1"
              >
                ...
                <div className="p-5 relative z-10">
                  <div className="flex items-start gap-4">
                    {/* Player number badge */}
                    {jugador.number && (
                      <div className="absolute top-3 right-3 w-8 h-8 bg-gradient-to-br from-brand to-brand-mid rounded-lg flex items-center justify-center shadow-lg shadow-brand/25">
                    ...
                    {/* Avatar with glow */}
                    <div className="relative">
                      <div className="absolute -inset-1 bg-gradient-to-r from-brand to-brand-mid rounded-2xl blur opacity-0 group-hover:opacity-50 transition-opacity" />
                      <Avatar className="relative w-18 h-18 border-3 border-white dark:border-gray-700 shadow-xl rounded-2xl">
                        ...
                        />
                        <AvatarFallback className="bg-gradient-to-br from-brand to-brand-mid text-white text-xl font-bold rounded-xl">
                    ...
                    <div className="flex-1 min-w-0 pt-1">
                      <h4 className="font-bold text-gray-900 dark:text-white text-lg truncate group-hover:text-brand transition-colors">
                        {jugador.name}
                      </h4>
        ...
        <div className="grid gap-6 md:grid-cols-2">
          {teamData.tournamentTeams.length === 0 ? (
            ...
            teamData.tournamentTeams.map((tt) => (
              <Card
                key={tt.id}
                className="group relative bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden rounded-2xl border border-gray-100 dark:border-gray-700 hover:border-brand/50 hover:-translate-y-1"
              >
                {/* Gradient accent bar */}
                <div className="h-1.5 bg-gradient-to-r from-brand via-brand-mid to-brand-2" />
                ...
                <CardContent className="relative z-10">
                  ...
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                    ...
                    <Badge className="bg-gradient-to-r from-brand to-brand-mid hover:from-brand-hover hover:to-brand-mid-hover text-white px-4 py-1.5 text-sm font-bold shadow-lg shadow-brand/25 border-0">
                      {tt.points} Pts
                    </Badge>
                  </div>
                </CardContent>
          ...
          <CardContent className="p-6">
            {teamData.partidos.length === 0 ? (
              <div className="text-center py-16">
                <div className="w-20 h-20 mx-auto mb-4 bg-gradient-to-br from-brand/20 to-brand-mid/10 rounded-full flex items-center justify-center">
                  <Play className="w-10 h-10 text-brand" />
                </div>
                <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">
