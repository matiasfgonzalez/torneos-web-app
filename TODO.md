...
- [x] **S8. Exportables (E:Bajo-Medio):** ✅ **Hecho (2026-07-17).** Tabla y fixture como PDF con branding, planteles en CSV.
  ...
  - **Gateado por plan — hecho (2026-07-17).** El export ahora se ofrece **solo si la liga tiene `exportPdf`** en su plan (`hasFeature`). UI: el `ExportMenu` no aparece en el detalle público ([TournamentDetailView](modules/torneos/components/TournamentDetailView.tsx)) ni en el panel admin ([Header](app/admin/torneos/[id]/components/Header.tsx)) sin la feature. Server (defensa de la URL directa): el CSV ([roster route](app/api/tournaments/[id]/export/roster/route.ts)) → 403; la vista de impresión ([print page](app/print/torneo/[id]/page.tsx)) → mensaje "no disponible en este plan" (no 404, porque el link puede estar compartido). El seed ya trae `exportPdf` en PRO y PREMIUM (FREE no), así que las ligas pagas lo conservan y solo FREE lo pierde. Mismo patrón que el gate de `liveMatch` (S6) y `orgNews` (S12): `hasFeature` en la UI (ocultar) y en el server (bloquear la URL). **Queda sin gatear `customBranding`** — es la única feature de la landing que todavía no está construida (sigue en el bloque ámbar de `/admin/planes`).
...
- [x] **S11. Billing/planes freemium:** ✅ **Hecho (manual, sin Mercado Pago) (2026-07-18).** N4 (planes/límites) + N5 (pagos manuales informados con comprobante) + gating de features por plan. Mercado Pago queda para después (el modelo ya tiene `externalId`/`method: MERCADOPAGO` preparados).
  ...
  - **Enforcement de plan cableado** (regla "no anuncies lo que no existe"): `assertPlanLimit` (torneos/equipos/miembros, N4) + `hasFeature` para features. Con el gating de S6/S8/S12, **todas las features anunciadas están enforced** salvo `customBranding` (única sin construir).
