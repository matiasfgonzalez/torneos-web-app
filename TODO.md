...
- [x] **S8. Exportables (E:Bajo-Medio):** ✅ **Hecho (2026-07-17).** Tabla y fixture como PDF con branding, planteles en CSV.
  ...
  - **Falta de S8:** (1) **no se abrió el diálogo de impresión real en un navegador** — se verificó el HTML/CSS del documento y que las tres descargas respondan, no el render final del PDF impreso (revisar márgenes/saltos con una impresión real antes de anunciarlo); (2) **el `ExportMenu` vive solo en el detalle público** del torneo, no en el panel admin (`/admin/torneos/[id]`) — el organizador llega igual desde la página pública, pero el acceso directo desde el panel queda pendiente; (3) el CSV es **solo planteles**; un export más rico para el organizador (con DNI, gateado por `orgAuth`, o planteles+stats) queda para una iteración; (4) no hay export de goleadores/estadísticas ni "todo el torneo en un ZIP" — se cubrió lo del enunciado (tabla, fixture, planteles).
