...
- [x] **Hecho (2026-07-21).** Noticias exponían el objeto `user` completo (email, `clerkUserId`, etc.). Se creó [modules/noticias/authorSelect.ts](modules/noticias/authorSelect.ts) (`newsAuthorSelect` = `id/name/imageUrl/createdAt`) y se aplicó a los 5 reads (`getAllNoticias`, `getNoticias`, `getNoticiaById`, `GET/PUT /api/noticias`, `GET /api/noticias/[id]`). El tipo `INoticia.user` pasó de `IUser` a `INewsAuthor` (sin PII). Se quitó el bloque "Email" del autor en [app/admin/noticias/[id]/page.tsx](app/admin/noticias/[id]/page.tsx) (ya no llega y era PII innecesaria — el autor siempre es ADMINISTRADOR). Torneos/equipos/jugadores ya no incluían `user` (verificado). `tsc` + `next build` en verde. **E:Bajo**
