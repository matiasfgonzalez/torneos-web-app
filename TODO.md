...
- ⚠️ **Verificación automática pendiente:** el entorno de shell (Bash/PowerShell) quedó temporalmente no disponible durante esta pasada (el clasificador de seguridad del harness no respondía) — no se pudo correr `tsc`/lint/`build`/smoke test como en el resto de las tareas de esta sesión. Se hizo una revisión manual línea por línea de los 7 archivos tocados; se encontró y corrigió en la revisión un error de tipos real (`TOURNAMENT_STATUS_SOLID_COLORS[tournament.status]` necesitaba el cast `as Record<string, string>` que ya usa `StatusBadge.tsx` para el mismo problema, porque `ITorneo.status` es `string` y no el enum `TournamentStatus`). **Correr `tsc`/`lint`/`build` antes de dar este ítem por verificado del todo.**
- [ ] Filtros como chips horizontales scrolleables en mobile (patrón app deportiva) con estado en URL.
- [ ] Migrar los links UUID restantes a `tournamentPublicPath` (FanHome, FavoritesTab, `/partidos`) sumando slugs a sus queries — mientras tanto dependen del redirect (roto solo en local por Node 24).
