...
- [ ] **USUARIO logueado:** home con torneos/equipos seguidos (`Favorite { userId, tournamentId?, teamId? }` — modelo no existe, hay que crearlo), notificaciones (depende de S5, no implementado), CTA "Creá tu liga" ✅ ya existe (`/crear-liga`). Perfil (`/profile`) hoy solo tiene info personal + "torneos organizados" (pensado para USUARIO=organizador); falta la versión "fan que sigue ligas". **No se hizo en esta pasada** — requiere migración de schema (`Favorite`), se prioriza junto con S5.
...
- [ ] **Admin (vos):** cola de pagos ✅ ya existía (`/admin/pagos`, aprobación); **pendiente:** CRUD de planes (hoy `Plan` solo se lee vía `GET /api/plans`, no hay UI de alta/edición), listado de organizaciones con estado/plan/último pago y acción suspender (no existe ninguna vista agregada de organizaciones — hoy ADMINISTRADOR solo ve usuarios y pagos sueltos), métricas SaaS (orgs activas, conversión FREE→PRO, ingresos del mes, torneos creados — no hay ninguna vista), "ver como organización" / impersonar (cero código, solo mencionado acá). Es el bloque más grande que queda de N10; conviene abordarlo junto con N3 (visibilidad de datos entre orgs) ya que ambos tocan el mismo modelo de "vista de plataforma vs. vista de org".
