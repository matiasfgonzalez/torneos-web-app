...
- [x] **Hecho (2026-07-21).** Noticias exponían el objeto `user` completo (email, `clerkUserId`, etc.). Se creó [modules/noticias/authorSelect.ts](modules/noticias/authorSelect.ts) (`newsAuthorSelect` = `id/name/imageUrl/createdAt`) y se aplicó a los reads vivos: `getNoticiaById` + `GET/PUT /api/noticias` + `GET /api/noticias/[id]` (la lista pública usa `fetch("/api/noticias")`). Las actions `getAllNoticias`/`getNoticias` también se tocaron pero resultaron **código muerto** y se eliminaron en B4. El tipo `INoticia.user` pasó de `IUser` a `INewsAuthor` (sin PII). Se quitó el bloque "Email" del autor en [app/admin/noticias/[id]/page.tsx](app/admin/noticias/[id]/page.tsx) (ya no llega y era PII innecesaria — el autor siempre es ADMINISTRADOR). Torneos/equipos/jugadores ya no incluían `user` (verificado). `tsc` + `next build` en verde. **E:Bajo**
- [x] **Hecho (2026-07-21).** Árbitros exponían PII (email/teléfono/DNI) sin auth: `GET /api/referees` scopea ahora por `getPanelOrgIds`/`orgScopeWhere` (anon → `[]`, miembro → sus orgs, ADMINISTRADOR → todas); `GET /api/referees/[id]` exige `requireApiOrgAccess(org, { allowCollaborator })` (anon/otros → 401/403); la action `getRefereeById` guardada con `authForReferee`. `getReferees` (action) ya estaba org-scoped. **E:Bajo**
...
- [x] **`sitemap.ts` + `robots.ts` — hecho (2026-07-21).** [app/robots.ts](app/robots.ts) indexa lo público y bloquea `/admin`, `/api`, `/profile` y áreas privadas, con ref al sitemap. [app/sitemap.ts](app/sitemap.ts) = rutas estáticas + ligas ACTIVA + torneos canónicos + equipos + jugadores + noticias publicadas.
  - ⚠️ **Fix del deploy (2026-07-22):** nació con `revalidate = 3600` (ISR) y **rompía el build de Vercel**: el ISR lo prerenderiza durante el build, donde no hay base, y el deploy entero fallaba con `Error occurred prerendering page "/sitemap.xml"` → `PrismaClientInitializationError`. Ahora es `dynamic = "force-dynamic"` (se arma por request; un sitemap no justifica atar el build a la base) **+ `try/catch`** que devuelve solo las rutas fijas si la BD no responde — un sitemap incompleto sirve, un 500 le dice al buscador que el sitio está roto. Verificado reproduciendo la falla: `next build` con `DATABASE_URL` inalcanzable ahora termina en **exit 0**, 57/57 páginas, 0 errores de prerender, y `/sitemap.xml` figura como `ƒ` (dinámica). Se auditó de paso que **ninguna** ruta estática (`/_not-found`, `/manifest.webmanifest`, `/robots.txt`, `/sso-callback`) toca la base.
...
- [x] **OG dinámica del resultado de partido — hecha (2026-07-22).** [opengraph-image.tsx](<app/(public)/partidos/[id]/opengraph-image.tsx>) para `/partidos/[id]`: es el link que más se comparte de todos —nadie manda "mirá la tabla", manda **el resultado**—. Se genera en el server en cada scrapeo, así que el mismo link muestra "VS + horario" antes de empezar y el marcador después. Contempla **penales** y **walkover** (el marcador solo no los explica), destaca **EN VIVO** en rojo, y cae a una tarjeta de marca si el partido no existe o su torneo fue eliminado. Query liviana aparte ([getMatchOgData.ts](modules/partidos/actions/getMatchOgData.ts)): la OG la pide un scraper y no tiene por qué pagar los seis niveles de relaciones de `getMatchById`. La fecha se formatea a mano y no con `Intl` — el runtime de `ImageResponse` no garantiza los locales y una fecha en inglés en una preview en español se nota. En `generateMetadata` **no** se declara `openGraph.images` a propósito (lo pisaría), y se sumó `canonical`.
- [x] **Redirect legacy 307 → 308 — hecho (2026-07-22).** `/torneos/[id]` → `/liga/[org]/[torneo]` usa `permanentRedirect`: la mudanza es definitiva y un 307 le decía al buscador "es temporal, seguí indexando la vieja". Con 308 el link juice transfiere a la canónica y la URL por UUID deja de competir con ella.
- **Verificado en el server de desarrollo:** la OG se sirve desde el `og:image` real de la página (Next le agrega un hash a la ruta, así que pedirla a mano da 404 — se extrajo del meta como haría un scraper): **HTTP 200, `image/png`, 1200×630**, magic bytes de PNG correctos, probado con un partido PROGRAMADO (variante "VS") y dos FINALIZADO (variante marcador). El redirect legacy devuelve **308** a la URL canónica. **M3 cerrada del todo.**
...
- [x] **Hecho (2026-07-21).** Pantalla `/admin/mi-liga` ([app/admin/mi-liga/page.tsx](app/admin/mi-liga/page.tsx) + [MiLigaClient.tsx](app/admin/mi-liga/MiLigaClient.tsx)) para editar logo + color de la liga contra `PATCH /api/org`, con su ítem `ownerOnly` en el nav del panel. El color queda gateado por `hasFeature(customBranding)` con upsell si el plan no lo incluye. Descripción original del hallazgo abajo (contexto).
...
- [x] **(Hallazgo C3, 2026-07-04) Formularios que no mandaban el `publicId` — noticias arreglado (2026-07-22).** El **editar** de noticias era el peor caso: no tenía subida de imagen en absoluto. Había un input suelto de "URL de Imagen" y un botón **"Subir Imagen" sin `onClick`** (botón muerto, prohibido por AGENT_RULES) — no se podía cargar una imagen desde la PC, y pegar una URL a mano guardaba solo `coverImageUrl`, nunca el `coverImagePublicId`, así que el asset quedaba huérfano por diseño. Reemplazado por `<CloudinaryUpload folder="noticias/covers">`, el mismo componente del resto del panel, con `coverImagePublicId` en el estado del form. Verificado con round-trip contra la BD (se escribe el publicId, se lee, se restaura). **Falta revisar el resto de los formularios con imagen** (`Team.logoPublicId` y demás) con el mismo criterio. **E:Bajo**
...
- [~] **B4. Código muerto (2026-07-21).** Corrido `npx knip` (ruidoso: muchos falsos positivos — barrels `index.ts`, primitivas UI, `sw.js` runtime). Borrados **13 archivos** verificados con 0 imports:
  ...
  - Ads/muertos: `components/BuscandoRivales.tsx`, `components/home-ads.tsx`, `components/video-ads.tsx`.
  - Diálogos viejos de torneo reemplazados por los `*Sheet` (A1/F3): `DialogAddEditMatch.tsx`, `DialogAddEditTeamPlayer.tsx`, `DialogAddEditTeamTournament.tsx`, `tournament-team-form.tsx` (en `app/admin/torneos/[id]/components/`).
  - Cadena muerta de noticias (la lista pública usa `fetch("/api/noticias")`; el barrel `@modules/noticias` no lo importa nadie): `modules/noticias/{index.ts, actions/getAllNoticias.ts, actions/getNoticias.ts, components/ListNoticias.tsx, components/Noticia.tsx, components/SkeletonNoticia.tsx}`.
...
- [x] ⚠️ **Soft 404 en toda la app — arreglado (2026-07-18).** `notFound()` renderizaba la 404 pero con **HTTP 200**. **La sospecha (Clerk) era falsa.** La causa real era el **streaming**: dos boundaries por encima de las páginas hacían *flush* del shell con 200 **antes** de que la página de detalle llegara a `notFound()` — (1) el `<Suspense fallback={null}>` del layout raíz y (2) `app/loading.tsx` (el loading global). Aislado con método: una ruta **sin match** daba 404 nativo, pero una ruta **con match + `notFound()`** daba 200; quitando los dos boundaries → 404.
  ...
  - **`noticias/[id]` — arreglado (2026-07-18).** Era el único detalle **client-fetched** (useEffect + `notFound()` desde el cliente = 200). Se refactorizó a **Server Component**: trae la noticia con [getNoticiaById](modules/noticias/actions/getNoticiaById.ts) y `notFound()` da 404 real; lo interactivo (compartir) se aisló en `ShareCard`. Bonus: (1) `generateMetadata` real (título + OG para compartir — antes **no tenía metadata**), y (2) la query pública filtra `published: true` + `deletedAt: null`, así **borradores y eliminadas dan 404** (no se muestran ni se indexan; el admin previsualiza desde `/admin/noticias`). Verificado: publicada → 200 con contenido y `<title>`; borrador → 404; inexistente → 404. **Todos los detalles públicos dan 404 correcto ahora.**
...
- [x] **S1b. Respetar los grupos ya asignados al generar el fixture (2026-07-22).**
  - **Detectado por el usuario antes de romper nada:** estaba armando el Mundial 2026 (48 equipos en los 12 grupos reales, cargados a mano) y preguntó si el generador iba a respetar esa asignación. **No la respetaba.** Con formato GRUPOS el generador siempre repartía por su cuenta y hacía `update({ data: { group } })` sobre cada equipo, pisando el sorteo. Y lo peor no era perder los grupos: **los partidos se arman sobre los grupos resultantes**, así que corregirlos después no habría arreglado el fixture — había que borrarlo y rehacerlo.
  - Nuevo `groupByExisting` en [lib/fixture/groups.ts](lib/fixture/groups.ts) + opción `existingGroups` en el generador. Cuando se usa, el plan **no devuelve `groupAssignments`**: esa es la garantía de que el server no tiene nada que escribir y no puede pisar el sorteo.
  - **Toggle "Usar los grupos ya asignados"** en el sheet, que **viene activado si todos los equipos ya tienen grupo** — el default seguro para quien ya hizo el sorteo. Si solo algunos lo tienen, avisa cuántos faltan en vez de fallar al generar. La cantidad de grupos se oculta cuando el toggle está activo, porque deja de aplicar.
  - Errores accionables desde la lógica pura: equipos sin grupo ("asignáselo antes de generar, o dejá que el sistema sortee") y grupos de un solo equipo.
  - **9 tests nuevos** ([tests/fixture/grupos-existentes.test.ts](tests/fixture/grupos-existentes.test.ts)), incluido el caso real: 12 grupos de 4 → **72 partidos, 6 por grupo, ningún cruce entre grupos**, y que sin la opción el comportamiento viejo no cambia. **Verificado además contra los datos reales del torneo del usuario** (48 equipos, 12 grupos) ejecutando el generador sin escribir: 72 partidos, 12 zonas, cero cruces, `groupAssignments` vacío.
  - 240 tests verdes, `tsc` + `eslint` + `build` limpios.
...
- [x] **S6. Live match center (E:Alto):** ✅ **Hecho (2026-07-17).** El público ve el partido actualizarse solo (polling); el admin ya cargaba eventos minuto a minuto.
  - **La carga admin ya existía** ([QuickMatchLoader](app/admin/partidos/[id]/cargar/QuickMatchLoader.tsx)): steppers de marcador, estado EN_JUEGO/ENTRETIEMPO/FINALIZADO y goles/tarjetas con minuto (`ManageGoals`/`ManageCards`). S6 construyó **la experiencia pública en vivo**, que faltaba (la ficha era estática y no auto-refrescaba).
  - **Polling, no SSE** (decisión): SSE en Vercel serverless tiene límites de duración y el proyecto ya usa polling (la campana de S5). Robusto y sin infra nueva. **Con conciencia de visibilidad** ([hooks/use-live-poll.ts](hooks/use-live-poll.ts)): si el hincha manda la app a segundo plano, deja de pegarle al server hasta que vuelve (ahorra datos/batería en la cancha) y refresca al instante al volver a foco.
  - **Estado live compartido** ([modules/partidos/utils/liveState.ts](modules/partidos/utils/liveState.ts)): un solo `buildLiveState` arma la cronología (goles+tarjetas por minuto, autogol para el rival) tanto para el SSR inicial como para el endpoint de polling → la pantalla no cambia de forma entre el primer render y las actualizaciones.
  - **Ficha del partido** ([LiveMatch](modules/partidos/components/LiveMatch.tsx), reemplaza el marcador+cronología estáticos de `MatchDetailView`): mientras el partido está EN_JUEGO/ENTRETIEMPO refresca desde `/api/matches/[id]/live` cada 15s; al terminar, **deja de pollear solo**. Banner "EN VIVO"/"Entretiempo" con pulso + minuto del último evento, caption "Se actualiza sola", y los eventos que llegan por polling entran con animación sutil (`motion-safe`). Árbitros/accesos siguen siendo SSR.
  - 🔑 **Reloj honesto, sin reloj falso:** no se inventó un cronómetro de pared (necesitaría kickoff + medio tiempo + 2º tiempo, y quedaría mintiendo un "73'" si el admin no cargó nada). Se muestra el **minuto del último evento cargado** — que es dato real que el admin ingresa.
  - **Hub "En vivo ahora"** ([LiveNowSection](modules/partidos/components/LiveNowSection.tsx)) arriba de `/partidos`: franja con los partidos que se juegan ahora (marcador + minuto), pollea `/api/matches/live` cada 20s y **no renderiza nada si no hay vivos**. De paso ENTRETIEMPO ahora cuenta como "en vivo" en el KPI y en las cards de la lista (antes solo EN_JUEGO).
  - **Endpoints compactos** ([/api/matches/[id]/live](app/api/matches/[id]/live/route.ts) y [/api/matches/live](app/api/matches/live/route.ts)): GET público, `no-store`, payload mínimo (nada de árbitros ni includes profundos). El hub filtra torneos no eliminados.
  - Verificado **de verdad** (script Prisma que puso un partido real en EN_JUEGO, consultó los endpoints y **restauró** el estado): `/api/matches/live` lo lista con su card (2-1, equipos, torneo); `/api/matches/[id]/live` devuelve status EN_JUEGO + score + eventos; la página `/partidos/[id]` SSR muestra el banner "En vivo" + "Se actualiza sola"; hub sin vivos → `[]`; partido inexistente → 404. `tsc` limpio, 0 errores de lint (solo warnings `<img>` preexistentes), build verde.
  - **Gateado por plan — hecho (2026-07-17).** El centro en vivo se ofrece **solo si la liga tiene `liveMatch`** en su plan. Sin la feature: la ficha del partido muestra el marcador pero **no se auto-actualiza** (ni polling ni banner "en vivo" — el estado "En juego" lo sigue diciendo el `StatusBadge`), controlado por el prop `liveEnabled` que baja de la página ([partidos/[id]/page.tsx](app/(public)/partidos/[id]/page.tsx)) hasta [LiveMatch](modules/partidos/components/LiveMatch.tsx). Server: `/api/matches/[id]/live` → 403; el hub `/api/matches/live` **filtra a las ligas con `liveMatch`** (lectura pura de la suscripción efectiva, sin crear suscripciones FREE como efecto de que un hincha mire `/partidos`). El seed trae `liveMatch` solo en PREMIUM, así que hoy solo esas ligas tienen vivo. `/admin/planes` reorganizó los switches: `exportPdf`/`liveMatch`/`orgNews` pasaron al bloque verde "ya enforced", solo `customBranding` queda en el ámbar "no construida".
  - **Falta de S6:** (1) **no se observó el polling en vivo en un navegador real** — se verificó que los endpoints reflejan el partido y que el componente renderiza el estado live, no el auto-refresh visual con dos pestañas; (2) **el minuto es el del último evento, no un cronómetro** (decisión de arriba) — si se quiere reloj real hace falta `liveStartedAt` + control de tiempos en el admin (migración + UI, otra iteración); (3) **la lista principal de `/partidos` no pollea** (solo el hub "En vivo ahora") — es un directorio, no una pantalla de vivo; (4) 15s de intervalo es un número fijo razonable, no configurable; (5) sigue siendo polling, no push/SSE — para "al toque" real haría falta SSE/WebSocket, fuera de alcance para serverless hoy.
...
- [x] **S8. Exportables (E:Bajo-Medio):** ✅ **Hecho (2026-07-17).** Tabla y fixture como PDF con branding, planteles en CSV.
  - **Enfoque sin dependencias nuevas:** el "PDF" es una **ruta imprimible** ([app/print/torneo/[id]/page.tsx](app/print/torneo/[id]/page.tsx)) con `window.print()` → el organizador elige "Guardar como PDF". Cero libs de PDF (el proyecto ya viene recortando deps, A9), determinístico y branded. El CSV es una **descarga real** vía route handler.
  - **Fuente de datos única** en [lib/export/tournament-export.ts](lib/export/tournament-export.ts) (`getTournamentExportData` + `buildStandings` + `buildRosterCsv`), así el PDF y el CSV nunca muestran cosas distintas. 🔑 **La tabla se ordena con el MISMO `makeStandingsComparator(tiebreakers)` que la web** ([lib/standings/config.ts](lib/standings/config.ts)) — el PDF nunca contradice a la pantalla, y respeta grupos (una tabla por grupo).
  - **Branding real en papel:** encabezado con logo + nombre de la organización, nombre del torneo, localidad y fechas. Se fuerza `print-color-adjust: exact` para que el gradiente de marca y las celdas salgan en la impresión (por defecto Chrome no imprime fondos). `@page A4`, `break-before: page` entre tabla y fixture, `thead` repetido por página.
  - **CSV** ([app/api/tournaments/[id]/export/roster/route.ts](app/api/tournaments/[id]/export/roster/route.ts)): `text/csv` + `Content-Disposition: attachment`, **BOM UTF-8** (Excel lee los acentos), una fila por jugador ordenada por equipo y dorsal. **Sin DNI** — expone lo mismo que ya es público (equipo, dorsal, jugador, posición, capitán); la ficha es global y el DNI nunca es público (N12/legal). GET público; torneo eliminado → 404.
  - **Menú "Exportar"** ([modules/torneos/components/ExportMenu.tsx](modules/torneos/components/ExportMenu.tsx)) en la cabecera pública del torneo, junto a Compartir: Tabla (PDF) · Fixture (PDF) · Planteles (CSV). Los PDF abren la vista imprimible en otra pestaña; el CSV descarga directo.
  - Verificado (producción, torneo real): CSV 200 con `attachment`/BOM/filas reales sin DNI; `/print/torneo/[id]?doc=tabla|fixture|todo` 200 con encabezado branded + tabla (equipos reales) + fixture + `print-color-adjust`/`page-break`; el trigger "Exportar" presente en la página del torneo (los ítems son portales de Radix, renderizan al abrir); 404 de torneo inexistente. `tsc` limpio, 0 errores de lint (solo warnings `<img>` preexistentes), build verde.
  - **Acceso desde el panel admin — hecho (2026-07-17).** El `ExportMenu` ahora también vive en la cabecera del detalle admin ([app/admin/torneos/[id]/components/Header.tsx](app/admin/torneos/[id]/components/Header.tsx)), junto a Editar/Eliminar. Se le agregó un `variant` al componente: `hero` (pastilla blanca translúcida sobre el hero oscuro público) y `surface` (botón outline de marca para el card claro/oscuro del panel), así el mismo menú se adapta a las dos superficies sin duplicar código. Verificado: build verde, el Export público sigue intacto (default `hero`, sin regresión), y los endpoints de export responden 200. El render logueado del panel no se ejercitó (requiere sesión Clerk), pero es el mismo componente ya verificado y el panel no rompe (404 protegido con curl, esperado).
  - **Gateado por plan — hecho (2026-07-17).** El export ahora se ofrece **solo si la liga tiene `exportPdf`** en su plan (`hasFeature`). UI: el `ExportMenu` no aparece en el detalle público ([TournamentDetailView](modules/torneos/components/TournamentDetailView.tsx)) ni en el panel admin ([Header](app/admin/torneos/[id]/components/Header.tsx)) sin la feature. Server (defensa de la URL directa): el CSV ([roster route](app/api/tournaments/[id]/export/roster/route.ts)) → 403; la vista de impresión ([print page](app/print/torneo/[id]/page.tsx)) → mensaje "no disponible en este plan" (no 404, porque el link puede estar compartido). El seed ya trae `exportPdf` en PRO y PREMIUM (FREE no), así que las ligas pagas lo conservan y solo FREE lo pierde. Mismo patrón que el gate de `liveMatch` (S6) y `orgNews` (S12): `hasFeature` en la UI (ocultar) y en el server (bloquear la URL). (`customBranding` — la última que faltaba — se construyó y gateó el 2026-07-18; ver abajo.)
  - **Falta de S8:** (1) **no se abrió el diálogo de impresión real en un navegador** — se verificó el HTML/CSS del documento y que las tres descargas respondan, no el render final del PDF impreso (revisar márgenes/saltos con una impresión real antes de anunciarlo); (2) el CSV es **solo planteles**; un export más rico para el organizador (con DNI, gateado por `orgAuth`, o planteles+stats) queda para una iteración; (3) no hay export de goleadores/estadísticas ni "todo el torneo en un ZIP" — se cubrió lo del enunciado (tabla, fixture, planteles).
...
- [x] **S11. Billing/planes freemium:** ✅ **Hecho (manual, sin Mercado Pago) (2026-07-18).** N4 (planes/límites) + N5 (pagos manuales informados con comprobante) + gating de features por plan. Mercado Pago queda para después (el modelo ya tiene `externalId`/`method: MERCADOPAGO` preparados).
  - **N5 ya venía casi entero** (modelo `Payment`, API POST/GET/PATCH, UI del owner en `/admin/plan` y del admin en `/admin/pagos`, notificaciones PAGO_INFORMADO/APROBADO/RECHAZADO, extensión de suscripción en transacción al aprobar). Lo que se agregó para dejar el flujo redondo:
    - **Datos de cobro configurables** (no más `GOLAZO.PAGOS` hardcodeado): campos `paymentAlias`/`paymentHolder`/`paymentInstructions` en `SiteSettings` (migración `datos_de_cobro`), editables por el admin en [/admin/configuracion](app/admin/configuracion/ConfiguracionClient.tsx) (sección "Datos de cobro"). La pantalla de pago del owner ([PlanClient](app/admin/plan/PlanClient.tsx)) los muestra; si no están cargados, avisa "la plataforma todavía no publicó los datos para transferir" en vez de un alias falso.
    - **Comprobante OBLIGATORIO** ([validador](lib/validators/payment.ts): `receiptUrl` pasó de opcional a `z.string().url()` requerido; el botón "Informar pago" se deshabilita sin comprobante). Es "manual informado **con comprobante**": sin él el admin no tiene qué verificar. **5 tests de Vitest** fijan la regla ([payment-schema.test.ts](tests/payments/payment-schema.test.ts)).
    - **Un solo pago PENDIENTE a la vez** ([POST /api/payments](app/api/payments/route.ts) → 409 si ya hay uno): evita duplicados y que el admin vea el mismo pago dos veces. El owner ve un banner "tenés un pago en revisión" y no puede informar otro.
  - **Enforcement de plan cableado** (regla "no anuncies lo que no existe"): `assertPlanLimit` (torneos/equipos/miembros, N4) + `hasFeature` para features. Con el gating de S6/S8/S12 **y customBranding (2026-07-18)**, **TODAS las features anunciadas están enforced** — no queda ninguna sin construir, y el bloque ámbar de `/admin/planes` desapareció.
  - Verificado: migración aplicada y columnas persisten (round-trip); 5 tests de validador verdes; `tsc` limpio, 0 errores de lint, build verde; páginas admin protegidas (404 anónimo, no 500) y `POST /api/payments` anónimo → 401. El flujo logueado completo (informar con sesión, aprobar) no se ejercitó con Clerk real — misma limitación que el resto de flujos auth-gated.
  - **Falta de N5/S11:** (1) **Mercado Pago** (automático) — a propósito para después; el modelo ya está preparado; (2) **no ejercité el flujo con sesión Clerk real** (informar→aprobar→suscripción activa) — verifiqué modelo, validador (tests), gating y protección de rutas; (3) no hay **recordatorio de vencimiento** (aviso N días antes de que expire la suscripción) — necesitaría el mismo cron que falta para "próximo partido" (S5); (4) el admin ve el comprobante como imagen — si suben un PDF no se previsualiza (Cloudinary lo sube, pero el `<Image>` no lo muestra); evaluar aceptar/mostrar PDFs.
- **customBranding (marca propia de la liga) — hecho (2026-07-18).** Era la **última feature de plan anunciada sin construir**. Ahora la liga tiene un **color de marca propio** (`Organization.brandColor`, migración `marca_propia_liga`) que se aplica en su página pública [/liga/[slug]](app/(public)/liga/[slug]/page.tsx) **en vez del violeta de GOLAZO**, y se le **saca la atribución "Hecho con GOLAZO"** (que sí ven las ligas sin la feature).
  - 🔑 **Cómo aplica el color sin JS ni migrar toda la página a mano:** se overridean las variables `--brand`/`--brand-mid`/`--brand-2`/`--brand-hover` en el wrapper de la página vía `style`. Funciona porque el `@theme inline` de `globals.css` hace que `bg-brand`/`text-brand`/`from-brand` usen `var(--brand)` **directo** (no un valor resuelto), así el override cascada a todo el subárbol. De paso se migraron los `#ad45ff`/`#a3b3ff` hardcodeados de esa página a tokens (deuda M6), que es lo que los hace tomar el color propio.
  - **Dónde se carga el color:** en el paso 1 del wizard [crear-liga](app/crear-liga/CrearLigaWizard.tsx) (el editor de perfil de la liga, junto al logo), con un color picker + hex. Validado como hex `#RRGGBB` ([organization.ts](lib/validators/organization.ts)); expuesto por `/api/org` y `getOrganizationBySlug`.
  - **Gate:** `hasFeature(org.id, "customBranding")` — sin la feature (o sin color cargado) la página usa el violeta de GOLAZO y muestra la atribución. El switch en `/admin/planes` pasó al bloque verde "enforced"; **el bloque ámbar "no construida" ya no existe**.
  - **Alcance v1:** solo la página **de la liga** (`/liga/[slug]`), no el detalle de torneo (`/liga/[slug]/[torneo]`, que es un archivo enorme con hexes hardcodeados — migrarlo es una iteración M6 aparte). Verificado end-to-end (script que prende/apaga la feature + el color): CON → `--brand:#FF6600` aplicado y atribución ausente; SIN → sin color propio y atribución presente. tsc/lint/build limpios.
- **Gating de features por plan (S6/S8) — hecho (2026-07-17).** `hasFeature` en UI (ocultar la afordancia) y en server (bloquear la URL directa). **exportPdf**: `ExportMenu` oculto sin la feature (público + admin); CSV → 403; vista de impresión → mensaje "no disponible". **liveMatch**: la ficha del partido no auto-actualiza sin la feature (prop `liveEnabled` de la página a `LiveMatch`); `/api/matches/[id]/live` → 403; el hub `/api/matches/live` filtra a las ligas con `liveMatch` (lectura pura de la suscripción efectiva, sin crear suscripciones FREE). `/admin/planes` reorganizó los switches: `exportPdf`/`liveMatch`/`orgNews` al bloque verde "ya enforced", solo `customBranding` en ámbar. Seed: `exportPdf` en PRO/PREMIUM, `liveMatch` solo PREMIUM. **Verificado end-to-end** (script que prende/apaga las features del plan): ON → CSV 200, print OK, live 200, hub incluye; OFF → CSV 403, print bloqueado, live 403, hub excluye.
- [x] **S12. Novedades de la liga (E:Medio, decisión D13):** ✅ **Hecho (2026-07-17).** El organizador publica novedades en su página pública `/liga/[slug]`, gateado por la feature de plan `orgNews`.
  - 🐞 **Bug encontrado y arreglado (2026-07-22): la feature nunca estuvo prendida para nadie.** Reportado por el usuario: con **Premium activo**, `/admin/novedades` mostraba "Novedades es una función de plan superior". Causa: `orgNews` se agregó al validador ([validators/plan.ts](lib/validators/plan.ts)), a la pantalla `/admin/planes` y al enforcement, **pero no al seed** — así que ningún plan la tenía en su JSON de features, y como `hasFeature` devuelve `false` ante una clave ausente, quedó apagada para todos. **No daba ningún síntoma**: no rompe, no loguea, solo niega. El organizador pagaba y la pantalla lo trataba como plan gratis.
  - **Arreglado en cuatro capas, no solo el dato:**
    1. **Catálogo único** [lib/constants/plan-features.ts](lib/constants/plan-features.ts) (`PLAN_FEATURES`, `FEATURE_KEYS`, `withAllFeatures`). Antes la lista estaba copiada en tres lugares —validador, seed y strings sueltos en `hasFeature`— que podían divergir, y divergieron.
    2. **`hasFeature` tipada con `FeatureKey`** en vez de `string`: un nombre mal escrito ahora **no compila**, antes era un `false` silencioso.
    3. **Definiciones de planes movidas a [prisma/plans.mjs](prisma/plans.mjs)** (datos sin escritura) y **el seed ahora fusiona en vez de pisar**: completa las claves que falten sin apagar lo que el admin haya prendido a mano desde `/admin/planes` — antes un reseed le borraba los cambios.
    4. **Test que ata las dos puntas** ([tests/plans/features.test.ts](tests/plans/features.test.ts)): todo plan declara todas las features del catálogo, ninguno declara una desconocida, y **un plan superior nunca pierde una feature del inferior** (invariante comercial: subir de plan no puede quitarte nada). **Verificado que el test falla de verdad**: se agregó una feature al catálogo sin tocar los planes y el test la marcó nombrando el plan culpable.
  - **Datos corregidos:** se corrió el seed y los tres planes recibieron `orgNews` (PREMIUM en `true`, FREE y PRO en `false`), conservando sus valores previos. Verificado sobre la liga real del usuario: `PREMIUM · ACTIVA · hasFeature(orgNews) → true`.
  - **Decisión pendiente para el usuario:** `orgNews` quedó **fuera de Pro**. Si las novedades tienen que entrar también en Pro, se prende desde `/admin/planes` sin tocar código.
  - (De paso) La nota de S12 que decía que `exportPdf` y `liveMatch` "todavía NO están gateadas" quedó **desactualizada**: hoy las cuatro features tienen su `hasFeature` en el camino real (verificado con grep sobre los call sites).
  - **Modelo propio `OrgPost`** (migración `novedades_de_la_liga`), NO reusa `News`: `organizationId`, título/resumen/contenido/portada, `published` + `publishedAt` **nullable seteado al publicar** (no `@default(now())` — evita el orden falso de borradores que tiene News), soft delete. Dueño = la organización (sin `authorId`: la ownership es la liga). Carpeta Cloudinary `novedades/covers` registrada en `ALLOWED_UPLOAD_FOLDERS` (C4).
  - **Feature de plan `orgNews` cableada de punta a punta** (regla AGENT_RULES "implementá → gateá → recién ahí el switch"): validador ([lib/validators/plan.ts](lib/validators/plan.ts)), toggle en `/admin/planes` (en su **propio bloque con nota verde "ya está enforced"**, separado del bloque ámbar de las features que todavía no se gatean), y bullet en la landing ([pricing-section](components/sections/pricing-section.tsx)). El **enforcement real** está en la creación: [POST /api/org-posts](app/api/org-posts/route.ts) llama `hasFeature(org.id, "orgNews")` → 402 con upsell si el plan no la incluye. **Editar/eliminar lo ya cargado NO se gatea** (nunca se bloquea gestionar datos existentes).
  - **Admin `/admin/novedades`** (ítem de nav nuevo, roles ADMIN+USUARIO): patrón nuevo — `<DataTable>` (búsqueda/filtro publicadas-borradores) + `<OrgPostForm>` con `FormSheet` + `SwitchField`/`ImageField` (no el dialog inline legacy de noticias). Toggle publicar desde el badge, eliminar con `AlertDialog`. Si el plan no incluye `orgNews`, muestra **upsell** y deshabilita el alta, pero deja ver/editar lo cargado. La org se resuelve con `getOrCreateOwnOrg` (la misma que apunta la API).
