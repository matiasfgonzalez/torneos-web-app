...
- [ ] **Botones muertos en el panel (encontrados 2026-07-13 al migrar las tablas — 🔴 son bugs visibles al usuario):**
  1. **Eliminar jugador:** el `AlertDialogAction` de `PlayersTable` **no tenía `onClick`** y `app/api/players/[id]/route.ts` **no tiene handler DELETE** (solo PATCH) → el botón abría el diálogo, confirmabas, y no pasaba nada. Lo **quité** en la migración en vez de dejar un botón que miente.
  2. **Eliminar equipo:** `DeleteTeamButton` hace `fetch('/api/team/${id}')` (singular) — **esa ruta no existe** (es `/api/teams`) — y `app/api/teams/[id]/route.ts` **tampoco tiene DELETE**. El botón sigue ahí y está roto.
  3. **"Programar primer partido"** en el estado vacío de `TabsMatches`: era un `<button>` sin `onClick`. Al migrar lo cablée al diálogo real de creación.
  - **No implementé los DELETE** porque la semántica es una decisión de negocio, no mecánica: ¿borrado físico o soft delete (`deletedAt` ya existe en `Player`)? ¿qué pasa con los goles/tarjetas/`TeamPlayer` del jugador y con el historial de partidos del equipo? Definir eso y después implementar los dos endpoints con su guard de rol/organización.
- [ ] **Deuda de lint preexistente en el panel (no la introduje, la encontré al verificar):** 47 errores en `app/admin` + `modules` — 17 `react-hooks/static-components` (todos en `player-form.tsx`: componentes declarados dentro del render), 7 `react-hooks/set-state-in-effect` (`DialogReferee`, `noticias/[id]`, `QuickMatchLoader`, `tournament-team-form`, `usuarios/[id]`, `usuarios`), 5 `react-hooks/immutability` y 18 `no-explicit-any` (casi todos en `modules/equipos`). Ninguno rompe el build hoy, pero `static-components` y `set-state-in-effect` sí causan renders en cascada y remontajes reales.
- [ ] **Formularios y diálogos:** los diálogos ya comparten el patrón visual (barra de acento + secciones `bg-gray-50 dark:bg-gray-800/50` + tokens), pero falta: loading en submit consistente, validación inline con mensajes en español, diálogos largos → `<Sheet>` lateral en desktop y fullscreen en mobile, autosave de borradores en torneos.
- [ ] **Navegación:** el indicador activo en el sidebar ya está (por prefijo, arriba). Falta: breadcrumbs en subpáginas (el componente `<PageHeader breadcrumbs>` ya los soporta, hay que pasárselos), command palette `Ctrl+K` con `cmdk`.
...
- [~] **USUARIO logueado:** ✅ **Implementado (2026-07-13):** modelo `Favorite` (migración `20260713024328_favorites`) — un usuario sigue un torneo o un equipo (exactamente uno de los dos FK, validado en la capa de acciones; los `@@unique([userId, tournamentId])`/`@@unique([userId, teamId])` no chocan entre sí porque Postgres no considera dos NULL iguales). [modules/favoritos/actions/favorites.ts](modules/favoritos/actions/favorites.ts): `toggleFavoriteTournament`/`toggleFavoriteTeam`, `getUserFavorites`, `getFavoritedIds`. Componente `FollowButton` ([modules/favoritos/components/FollowButton.tsx](modules/favoritos/components/FollowButton.tsx), 3 variantes: `icon`/`full`/`hero` para fondos oscuros fijos) integrado en el header público de torneo (`HeaderTorneo.tsx`) y de equipo (`PublicTeamHeader.tsx`) — si no hay sesión, redirige a `/sign-in` en vez de fallar. **Home personalizado** (`app/page.tsx`): si el usuario está logueado, `/` deja de mostrar la landing de marketing y muestra `FanHome` (torneos/equipos seguidos con opción de dejar de seguir, CTA permanente "Creá tu liga" o "Ir a mi panel" según si ya tiene organización, accesos rápidos de descubrimiento) — la landing sigue intacta para visitantes anónimos. **Perfil**: nueva tab "Favoritos" en `/profile` con la misma lista.
  - **Pendiente:** notificaciones (depende de S5, no implementado — sin cambios).
