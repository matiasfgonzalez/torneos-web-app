...
const LightPillar: React.FC<LightPillarProps> = ({
  ...
  // El soporte de WebGL se resuelve en el inicializador del estado, no en un
  // efecto: es una capacidad del navegador que no cambia durante la vida del
  // componente, y detectarla con `useEffect` + `setState` encadena un render
  // extra (react-hooks/set-state-in-effect). En el server se asume que sí hay
  // soporte; el primer render del cliente corrige si hace falta.
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Camino de error, no de sincronización: el contexto WebGL no se pudo
      // crear, así que el componente se apaga y cae al fondo estático. Corre
      // una sola vez y no vuelve a entrar (el efecto sale con `webGLSupported`
      // en false), así que no hay cascada de renders que evitar.
