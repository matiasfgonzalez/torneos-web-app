...
const TeamsCarousel = (props: PropsTeamsCarousel) => {
  ...
  const nextSlide = () => {
    setCurrentIndex((prev) =>
      prev + teamsPerView >= tournamentTeams.length ? 0 : prev + teamsPerView
    );
  };
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>
              {tournamentTeams.length} equipos compiten en este torneo
            </CardDescription>
          </div>
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              size="sm"
              onClick={nextSlide}
              disabled={currentIndex + teamsPerView >= tournamentTeams.length}
      ...
      <CardContent>
        ...
        <div className="flex justify-center mt-4 gap-2">
          {Array.from({
            ...
            <button
              key={index}
              onClick={() => setCurrentIndex(index * teamsPerView)}
              className={cn(
                "w-2 h-2 rounded-full transition-all duration-300",
                Math.floor(currentIndex / teamsPerView) === index
                  ? "bg-primary w-6"
